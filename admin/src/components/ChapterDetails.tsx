...
//   try {
//     const res = await updateChapter({ id, data: update });
//     // Kiểm tra trực tiếp data.success
//     if (res.data?.success) {
...
import type { Chapter } from "../../../home/src/stores/interfaces/chapter";
...
export default function ChapterDetails({ id, open }: ChapterDetailsProps) {
  // ============================
  //  GET CHAPTER DATA trực tiếp
  // ============================
  const { data: chapter, isLoading, isError, refetch } = useGetChapterByIdQuery(
    id,
    {
      skip: !id,
      selectFromResult: (result: {
        ...
        data: result.data?.data, // chỉ lấy object chapter
  ...
  // ============================
  //  LOCAL STATE giữ dữ liệu input
  // ============================
  const [update, setUpdate] = useState<Partial<Chapter>>({});
    ...
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center">
      <div className="relative bg-white w-4/5 max-h-[90vh] rounded-2xl p-10 shadow-lg overflow-hidden">
        ...
        <div className="flex flex-col gap-6 max-h-[75vh] overflow-auto pr-3">
          ...
          <div className="flex gap-6">
            <div className="flex flex-col gap-1 w-3/4">
              ...
              <input
                id="name"
                type="text"
                value={update.name ?? chapter.name}
                onChange={handleChange}
                placeholder="Nhập họ tên"
                className="border border-blue-500 rounded-lg px-3 h-10 text-blue-700"
              />
            ...
            <div className="flex flex-col gap-1 w-1/4">
              ...
              <select
                id="status"
                title="status"
                value={update.status ?? chapter.status}
                onChange={handleChange}
                className={`border border-blue-500 rounded-lg px-3 h-10 font-bold
                  ${
                    (update.status ?? chapter.status) === "active"
                      ? "text-green-600"
                      : (update.status ?? chapter.status) === "locked"
                      ? "text-red-600"
                      : "text-orange-500"
                  }
                `}
          ...
          <div className="flex gap-6">
            <div className="flex flex-col gap-1 w-3/4">
              <label className="font-semibold text-blue-700">Đoàn trực thuộc</label>
              <input
                id="affiliated"
                type="text"
                value={update.affiliated ?? chapter.affiliated}
                onChange={handleChange}
                placeholder="Nhập affiliated"
                className="border border-blue-500 rounded-lg px-3 h-10 text-blue-700"
              />
            ...
            <div className="flex flex-col gap-1 w-1/4">
              <label className="font-semibold text-blue-700">Ngày thành lập</label>
              <input
                id="establishedAt"
                title="date"
                type="date"
                value={update.establishedAt ?? chapter.establishedAt?.substring(0, 10)}
                onChange={handleChange}
                className="border border-blue-500 rounded-lg px-3 h-10 text-blue-700"
              />
            </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              id="address"
              type="text"
              value={update.address ?? chapter.address}
              onChange={handleChange}
              placeholder="Nhập địa chỉ"
              className="border border-blue-500 rounded-lg px-3 h-10 text-blue-700"
            />
          ...
          <div className="flex flex-col gap-2">
            ...
            {chapter.fullname ? (
              <div className="flex items-center gap-3 text-blue-900">
                <img
                  src={chapter.avatar?.path || avatar}
                  alt={chapter.fullname}
                  className="w-12 h-12 rounded-full object-cover"
                />
                <p>{chapter.fullname}</p>
              </div>
            ) : (
              <p>Không</p>
            )}
